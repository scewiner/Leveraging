...
from thumt.layers.leverage_nn import linear,gate_sum
...
def multiplicative_attention(queries, keys, values, bias,local_bias, keep_prob=None,
                             name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
                       values=[queries, keys, values, bias,local_bias]):
        # shape: [batch, heads, length_q, length_kv]
        ...
        if keep_prob is not None and keep_prob < 1.0:
            weights = tf.nn.dropout(weights, keep_prob)
            local_weights== tf.nn.dropout(local_weights, keep_prob)

        global_outputs = tf.matmul(weights, values)
        local_outputs = tf.matmul(local_weights,values)
        outputs = gate_sum(local_outputs,global_outputs,queries)

        return {"weights": weights, "outputs": outputs}


def multihead_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, keep_prob=None,local_bias=None, output=True,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, local_bias,keep_prob)
