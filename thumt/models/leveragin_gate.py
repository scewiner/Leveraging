...
def transformer_encoder(inputs, bias, local_bias, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
                           values=[inputs, bias,local_bias]):
        x = inputs
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                if layer<params.layers:
                    with tf.variable_scope("self_attention"):
                        y = layers.leverage_att.multihead_attention(
...
def encoding_graph(features, mode, params):
    ...
    local_bias = get_local_bias(tf.shape(inputs)[1],params.wins)

    if params.residual_dropout:
        keep_prob = 1.0 - params.residual_dropout
        encoder_input = tf.nn.dropout(encoder_input, keep_prob)

    encoder_output = transformer_encoder(encoder_input, enc_attn_bias,local_bias, params)
...
class Transformer(interface.NMTModel):
    ...
    def get_training_func(self, initializer,regularizer=None):
        def training_fn(features, params=None, reuse=None):
            ...
            with tf.variable_scope(self._scope, initializer=initializer,regularizer=regularizer,
                                   reuse=reuse):
